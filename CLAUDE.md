...
# Bounded-context deep research with Layer-3 subagent FAN-OUT (VERIFIED 2026-06-22 on L4) — needs BRAVE_API_KEY in ~/.env
# The LEAD delegates each sub-question to an ISOLATED sub-agent (sessions_spawn/sessions_yield) so raw web
# pages stay in the child context; the lead stays bounded (compactionCount 0) and writes a cited table.
# (T4 fee-free uses Layer-1 pruning instead: --config configs/lfm2_ollama_research.json.)
bash bin/colab_openclaw_diffusiongemma.sh --gpu L4 \
  --config configs/diffusiongemma_research.json --task examples/web_research_fanout.json --out ./runs/fanout

# Citation-backed DEEP-RESEARCH REPORT (the deep-research skill ported from wg-automation's
# claude-deep-research-skill, optimized for DiffusionGemma) — needs BRAVE_API_KEY in ~/.env.
# shared-session: research phases (scope/retrieve/triangulate/critique) then PACKAGE split across 5
# section-sized turns -> a fully-cited research_result.md. Each turn = ONE cited section (≤2048 tok);
# the evidence ledger (memory/ev-NN-*.md + memory/_citations.md) is bundled under openclaw_state/memory/.
bash bin/colab_openclaw_diffusiongemma.sh --gpu L4 \
  --config configs/diffusiongemma_deepresearch.json --task examples/web_research_citation.json --out ./runs/deepresearch
```

`--keep-session` leaves the Colab runtime up after download (default tears it down). Pair it with **`--reuse-session`** to amortize the expensive DiffusionGemma cold start (model download + vLLM load, ~20 min): run 1 cold-starts with `--keep-session` (leaving the L4 **warm**), and subsequent runs add `--reuse-session` (same `--session NAME`) to **skip both `colab new` and the bootstrap** and run only the task — turning a ~32-min run into a ~10-min one. `--reuse-session` attaches by NAME (no `colab new`, so no duplicate-runtime trap), stores the session handle in a stable `./runs/.sessions/<name>.json` (so a different `--out` still finds the same VM), health-checks the warm backend before running, and respects `--keep-session` for whether to tear down after (omit it on the last run). Only the TASK varies across reuse runs — serve-affecting config changes need a fresh cold start. Reuse only nets compute-unit savings if you reuse promptly (idle billing has a break-even ~ one cold start); full mechanism + the cold-vs-warm cost model is in **`docs/warm_session_reuse_and_costs.md`**. The launcher runs `scripts/self_test.py` automatically before provisioning, so a self-test failure aborts the run.
...
2. `prompt` (worker) — one `openclaw infer model run … --json`, polled via `prompt_status`. For `task.mode == "research"` the launcher instead runs the `task` worker (multi-step deep research → `research_result.md`; `orchestration:"shared-session"` runs the steps sequentially in one session, `orchestration:"subagent-fanout"` runs one lead turn that delegates each sub-question to an isolated `sessions_spawn` child), polled via `task_status`.
...
- **Config JSON** (`configs/*.json`) drives everything model-side via a `serve` block: `serve.backend` (`llama_cpp` | `vllm` | `ollama`), `serve.host/port/startup_timeout_seconds`, and a backend sub-block — `serve.llama_cpp.{wheel, wheel_index, server_args}` with `model.{id, gguf_repo, gguf_file}`, **or** legacy top-level `vllm.{serve_args, install_command, …}` (the DiffusionGemma config's `serve_args` carry diffusion-specific flags `--diffusion-config`/`--hf-overrides`/`--generation-config vllm`). `openclaw.compat.{requiresStringContent, supportsTools, maxTokens, contextWindow}` supplies the infer-fixes. For the `ollama` backend: `serve.ollama.{num_ctx, install_timeout_seconds, download_timeout_seconds}` with `model.id` an Ollama tag (e.g. `lfm2.5:8b`). The web-search/identity wiring is config-driven too: `openclaw.web.{enabled, provider, plugin_package, max_results, code_mode, lean_workspace}` (installs the brave plugin, enables web tools, trims the workspace) and `openclaw.identity.{name, email?}` (seeds `~/.openclaw/workspace/USER.md`). For bounded-context deep research, `openclaw.context.{pruning, reserve_tokens_floor, reserve_tokens, mid_turn_precheck, tool_result_max_chars, memory_search_provider}` (`_configure_context`) turns ON `contextPruning` and LOWERS the compaction reserve so accumulating multi-step tool results don't overflow a small window (VERIFIED — see `docs/validation_findings.md`). Configs: `llama_lfm2.json` (validated T4 default — LFM2.5-8B-A1B), `lfm2_ollama_web.json` (T4 deep-research with live Brave web search via `ollama`), `llama_qwen9b.json` (alt; Qwen3.5-9B is unstable under llama.cpp on T4), `llama_smoke.json` (cheap 0.5B), `diffusiongemma_nvfp4.json` (vLLM/L4), `diffusiongemma_web.json` (L4 + live web search), `lfm2_ollama_research.json` (T4 bounded-context deep research — Layer-1 pruning), `diffusiongemma_research.json` (L4 bounded-context + Layer-3 subagent fan-out — VERIFIED), `diffusiongemma_deepresearch.json` (L4 citation-backed deep-research REPORT — drives the ported deep-research skill; same bounded-context knobs as `diffusiongemma_research.json`), `smoke_test_tiny.json` (vLLM 0.5B).
- **Task JSON** drives the request: `prompt`, `transport` (`gateway` | `local` — `local` = direct infer, no `--gateway`, the robust path), `timeout_seconds`. For autonomous runs set `mode: "research"` with a `steps` list (+ `topic`, optional `step_timeout_seconds`); see `examples/research_task.json`. For deep research that accumulates many tool results, `orchestration` selects the strategy: `shared-session` (default — one session across steps, pair with the `openclaw.context` Layer-1 pruning above) or `subagent-fanout` (Layer-3 — the lead fans each sub-question to an isolated `sessions_spawn` child so raw pages stay out of its context; VERIFIED on L4, see `examples/web_research_fanout.json`). Fan-out note: `openclaw agent --local --json` can hang after answering once subagents are spawned, so the remote recovers the lead synthesis from the server-side trajectory and judges success on captured text, not the CLI returncode. **Multi-LEVEL fan-out** (a LEAD→COORDINATOR→leaf coordinator tier, gated on `openclaw.fanout.multilevel`) is **DISABLED by default**: verified 2026-06-23 that the tier *forms* reliably but *never completes* on the available models (DiffusionGemma-26B orchestrates but is too slow; LFM2.5-8B is too weak to spawn the tier; batching breaks the diffusion model) — the default fan-out is flat single-level. Set `openclaw.fanout.multilevel:true` to re-enable. See `docs/validation_findings.md`. The native deep-research methodology lives in ONE OpenClaw skill — checked in at `skills/deep-research/SKILL.md` and shipped as the `DEEP_RESEARCH_SKILL` constant the remote installs to `~/.openclaw/skills/deep-research/` (`self_test` asserts the two stay byte-in-sync). It was **ported from wg-automation's `claude-deep-research-skill` and optimized for the small DiffusionGemma window**: full citation-integrity discipline (every claim cited `[N]`, complete bibliography, no fabrication, source-as-data trust boundary), an append-only evidence ledger in `memory/ev-NN-*.md` recalled via `memory_search`/`memory_get` (replaces the original's Python citation/evidence scripts, which don't exist on the VM), and PROGRESSIVE one-section-per-turn report writing — each step's reply IS one report section and the harness's per-step append to `research_result.md` assembles the report, so no single turn ever exceeds the 2048-token cap (`examples/web_research_citation.json` splits PACKAGE across 5 section-sized turns). The ev-note ledger is bundled under `openclaw_state/memory/` for offline citation audit. Heavyweight source features that don't port (HTML/PDF/WeasyPrint export, `search-cli`/Exa, the helper scripts, 20k-word single-shot reports, true-concurrent fan-out) are intentionally dropped — see **`docs/deep_research_port.md`** (curated reference) and `docs/validation_findings.md` (live log).
