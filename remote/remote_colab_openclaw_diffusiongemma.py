...
def _snapshot_openclaw_state() -> None:
    ...
    try:
        ...
        # Also copy the agent's externalized EVIDENCE LEDGER (memory/*.md ev-notes + _citations.md)
        # so the deep-research citation trail is auditable offline — each report [N] should map to a
        # saved ev-NN note. Best-effort over the likely roots (workspace + per-agent), deduped by name.
        mem_roots = [ws, Path(os.path.expanduser('~/.openclaw/workspace')),
                     Path(os.path.expanduser('~/.openclaw/agents'))]
        copied = set()
        for root in mem_roots:
            if not root.exists():
                continue
            for md in root.glob('**/memory/*.md'):
                if md.name in copied:
                    continue
                copied.add(md.name)
                d = dst / 'memory'
